...
class mysql
{
    ...
    public function connect()
    {
        ...
        if(! $this->link) {
           $message =  strtr('Could not connect: {error}' ,array('{error}'=>mysql_error()));
    ...
    public function query($sql)
    {
        ...
        if(! $query) {
           $message =  strtr('mysql query error:{error}:{link}:{sql}.' ,array('{error}'=>mysql_errno($this->link),'{link}'=>mysql_error($this->link),'{sql}'=>$sql));
           throw new Exception($message);
        }
        return $query;
